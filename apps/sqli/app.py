"""sqli.vulnlab.dev — entrypoint.

Mounts each SQLi lab as a Flask blueprint. Mirrors the ssrf/xss app shape:
/, /source/<slug>, /meta/<slug>, /meta/, robots, security.txt.

DB connection is opened per-request via apps.sqli.db.get_conn and torn
down on teardown_appcontext. Credentials come from env vars supplied by
the systemd EnvironmentFile= directive.
"""
...
def create_app() -> Flask:
    ...
    def robots():
        return (
            "User-agent: *\nAllow: /\nSitemap: https://sqli.vulnlab.dev/sitemap.xml\n",
            200,
    ...
    def security_txt():
        body = (
            ...
            "Canonical: https://sqli.vulnlab.dev/.well-known/security.txt\n"
